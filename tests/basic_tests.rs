...
fn test_af_xdp_protobuf_shape_validation() {
    ...
    transmit_packet(&mut tx_q_dev1, &umem_dev1, &mut frame_descs_dev1[0], &valid_packet);
    ...
    transmit_packet(&mut tx_q_dev1, &umem_dev1, &mut frame_descs_dev1[1], &invalid_packet);
...

    {
        ...
        assert_eq!(result.err(), Some(ValidationError::Proto(ProtoError::ShapeValueInvalid)));
    }
}
