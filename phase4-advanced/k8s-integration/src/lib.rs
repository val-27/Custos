...
pub fn send_fds(stream: &UnixStream, fds: &[RawFd]) -> io::Result<()> {
    ...
    let cmsg_len = unsafe { libc::CMSG_LEN((fds.len() * std::mem::size_of::<RawFd>()) as libc::c_uint) };
...
pub fn recv_fds(stream: &UnixStream, fds: &mut [RawFd]) -> io::Result<usize> {
    ...
    let cmsg = unsafe { libc::CMSG_FIRSTHDR(&msg) };
    if cmsg.is_null() {
        return Err(io::Error::new(io::ErrorKind::Other, "No control message received"));
    }
    ...
    unsafe {
        if (*cmsg).cmsg_level != libc::SOL_SOCKET || (*cmsg).cmsg_type != libc::SCM_RIGHTS {
            return Err(io::Error::new(io::ErrorKind::Other, "Received invalid control message"));
        }
        
        let data_ptr = libc::CMSG_DATA(cmsg) as *const RawFd;
        let num_fds = ((*cmsg).cmsg_len - libc::CMSG_LEN(0)) as usize / std::mem::size_of::<RawFd>();
...
pub fn receive_socket_fd(socket_path: &str) -> Result<std::os::unix::io::RawFd, String> {
    ...
    tracing::info!("Successfully received {} file descriptors over UDS SCM_RIGHTS", count);
    Ok(fds[0])
}
